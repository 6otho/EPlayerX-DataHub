...
> 基于 **Cloudflare Workers + Cloudflare R2 + GitHub API** 构建，支持 6 大追剧周更表自动分发、TMDB/MDBList 多源数据脱水抓取、Telegram 机器人双向控制、**1~7 天自定义周期调度**，以及 **GitHub `config.ts` 智能代码增量合并 (V2 标准)**。
...
- 📊 **四大权威评分与精准分级体系**：
  - 聚合 **TMDB、IMDb、Trakt、烂番茄 (Rotten Tomatoes)** 四大平台权威评分；
  - 自动提取中港台及欧美影视内容分级（如 `限`、`辅15`、`辅12`、`护`、`普`、`III`、`TV-MA`、`R` 等）。
- 🔑 **MDBList 多 API Key 自动顺延熔断**：
  - 环境变量支持填入多个 MDBList Key（用英文逗号 `,` 隔开）；
  - 单号用满 1000 次触发限流（429/401）时，**系统自动秒级切换至下一个备用 Key** 继续工作，大批量同步不中断。
- 🎯 **一键提取评分/图标/清空简介**：
  - 定向对勾选影片提取高清透明 Logo、带字剧照与纯净无字图；
  - **自动彻底抹除剧情简介（`overview` 等长文本）**，保持数据精简无冗余；
  - **100% 智能保留并补齐题材分类（`genre_ids` 动作/剧情/科幻等标签）**。
- ⏱️ **LED 控制台 1~7 天自定义周期调度**：
  - 实时 LED 动态监视屏，显示待命/同步状态、时钟与执行进度；
  - 支持自由设定 **每 1~7 天间隔** 以及 **每日 00:00~23:00** 任意整点自动全量抓取；
  - 支持一键测试（`🚀 立即测`）与后台暂停/恢复（`⏸ 暂停`）。
- 🛡️ **分类隔离黑名单与五维资产库**：
  - 支持将指定影片加入“当前分类黑名单”，后续该分类同步时自动跳过，绝不污染全局数据；
  - 支持可视化切换正标海报、纯净无字轮播海报、透明 Logo 与横版剧照。
- 🛠️ **定向入库引擎 (Direct Injection)**：支持直接输入影片名称/TMDB ID，或直接粘贴包含“周一/星期一”的纯文本或 Excel 表格排期，智能正则识别并一键入库。
...
| 环境变量名称 | 是否必填 | 类型 | 说明 | 示例值 |
| :--- | :---: | :--- | :--- | :--- |
| `ADMIN_SECRET` | **必填** | Secret | 后台控制台登录密码，以及 API 鉴权密钥（自定义设置）。 | `your_admin_password` |
| `WORKER_URL` | **必填** | Text | 你的 Worker 自定义绑定域名或默认分配域名（用于生成内部图片与数据链接）。 | `https://your-worker-domain.com` |
| `GITHUB_REPO` | **必填** | Text | 你的 EPlayerX 首页代码仓库路径（格式为 `用户名/仓库名`）。 | `your_username/your_repo` |
| `GITHUB_PATH` | **必填** | Text | `config.ts` 在 GitHub 仓库中的相对路径。 | `src/config/config.ts` |
| `GITHUB_TOKEN` | **必填** | Secret | 用于向 GitHub 仓库推送代码的 **Personal Access Token**。 | `ghp_your_github_token` |
| `TMDB_ACCESS_TOKEN` | **必填** | Secret | TMDB API 读访问令牌（v4 Auth），用于抓取海报、剧照和 Logo。 | `eyJhbGciOiJIUzI1NiJ9...` |
| `MDBLIST_API_KEY` | 选填 | Secret | MDBList API Key（用于提取烂番茄/Trakt评分）。**支持多个 Key 用英文逗号 `,` 隔开以实现自动顺延**。 | `key1,key2,key3` |
| `OMDB_API_KEY` | 选填 | Secret | OMDb API Key（用于备用获取 IMDb 与烂番茄评分）。 | `your_omdb_key` |
| `TRAKT_CLIENT_ID` | 选填 | Text | Trakt.tv API 客户端 ID（用于欧美剧集榜单抓取与评分备用）。 | `your_trakt_client_id` |
| `TG_BOT_TOKEN` | 选填 | Secret | Telegram 机器人的 Token（用于控制台交互与进度通知）。 | `123456789:ABCdefGhIJKlm...` |
| `TG_CHAT_ID` | 选填 | Text | 接收 Telegram 通知以及限制操作权限的你个人的 TG Chat ID。 | `123456789` |
...
4. 复制以 `eyJhbGci...` 开头的密钥。

### 3. 🍅 获取与配置 `MDBLIST_API_KEY`（多 Key 自动故障顺延）
1. 注册并登录 [MDBList 官网](https://mdblist.com/)。
2. 进入个人中心 **Preferences**，复制你的 API Key。
3. **多账号配置方式**：若单个免费账号的 1000 次配额不够用，可准备多个账号的 Key，在 Cloudflare 后台中用英文半角逗号 `,` 连接填入：
   ```text
   key_1_value,key_2_value,key_3_value
   ```
   系统会默认使用第 1 个 Key，一旦遇到配额耗尽或 429 报错，**将自动无感顺延切换到下一个 Key** 继续执行。

### 4. 🍿 获取 `OMDB_API_KEY`（选填）
1. 打开 [OMDb API 官网](https://www.omdbapi.com/apikey.aspx)。
2. 选择 **FREE** 方案，输入邮箱即可收到专属 API Key。
...
1. **获取 Token**：在 Telegram 中搜索 `@BotFather`，发送 `/newbot`，按照提示创建机器人，获取形如 `123456789:ABC...` 的 Token。
2. **获取 Chat ID**：在 Telegram 中搜索 `@userinfobot`，发送 `/start`，获取数字格式的 **Id**（如 `123456789`）。
...
3. 存储桶名称输入：`eplayerx-data`（可自定义），点击创建。
...
2. 参照上方的《环境变量配置清单》，将 `ADMIN_SECRET`、`GITHUB_TOKEN`、`TMDB_ACCESS_TOKEN`、`MDBLIST_API_KEY` 等变量逐一填入。
...
2. 输入你的自定义域名（例如 `homepage.your-domain.com`）。
3. 确保你的环境变量 `WORKER_URL` 的值与绑定的域名完全一致：`https://homepage.your-domain.com`。
...
* **LED 状态栏功能**：
  * `🗓️ 周期`：支持自由切换 `每 1 天` 到 `每 7 天` 自动更新一次。
  * `⏰ 时间`：选择每日在 `00:00` ~ `23:00` 哪一个整点启动。
  * `🚀 立即测`：立即全速跑完所有任务进行测试，不受设定周期的天数限制。
  * `⏸ 暂停`：一键暂停或恢复后台自动轮询。

### 2. 评分提取与简介清空
* 勾选列表中的影片卡片（支持多选），点击右上角 **`🎯 提取评分/图标/去简介`**：
  * 系统会自动提取所选影片的烂番茄、Trakt、IMDb 评分、分级标签和 Logo；
  * **自动删除该条目的剧情简介字段**（保持列表轻量，避免冗长文本占位）；
  * **完好保留动作/剧情等题材分类 (`genre_ids`)**。

### 3. 数据同步与分类隔离黑名单
* **单榜/批量同步**：支持单分类极速抓取或勾选多个榜单批量全自动同步。
* **从列表移除 / 加入分类黑名单**：
  * 点击卡片上的红色垃圾桶：仅从当前大盘列表中移除（下次全量同步时可能重新收录）；
  * 点击卡片上的盾牌图标：**加入当前分类的黑名单**，后续后台自动同步时将永远跳过此片，且不影响其他分类收录。

### 4. 可视化多维选图器
卡片下方提供 4 组快捷选图入口：
* **`轮播`**：手动指定用于精选卡片/大轮播的【纯净无字竖海报】；
* **`剧照`**：手动选择高清横版背景或带字剧照；
* **`竖`**：手动指定列表卡片专用的【官方艺术字正标海报】；
* **`标`**：手动选择或输入透明高清 Logo（亦可选择放弃图片强制生成纯白 SVG 文字标）。

### 5. 排版与 GitHub 增量推送
1. 点击右上角 **`🎨 排版与智能同步`**。
2. 长按左侧 ☰ 上下拖动可调整前端卡片展示顺序，右侧下拉框可切换 `竖版海报 (poster-list)`、`精选大图 (hero-list)` 或 `新番日历合集 (collection-list)`。
...
> **答**：准备备用 MDBList 账号，将获取的 Key 在环境变量 `MDBLIST_API_KEY` 中用英文半角逗号隔开填入（如 `key1,key2`）。系统在提取评分遇到限流时会自动无缝切换至下一个 Key。

#### Q3：推送后 GitHub 的 `config.ts` 打包报错或格式异常怎么办？
> **答**：本项目后台内置了 **括号平衡算法** 与 **全局语法清洗引擎**，在提交给 GitHub 时会自动校验并剔除无效逗号，确保生成的 TypeScript 语法 100% 合法。
